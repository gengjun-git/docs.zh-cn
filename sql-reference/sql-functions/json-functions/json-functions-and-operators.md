...
| [JSON_KEYS](../../sql-functions/json-functions/json-processing-functions/json_keys.md)   | 返回 JSON 对象中所有最上层成员 (key) 组成的数组。      | SELECT JSON_KEYS('{"a": 1, "b": 2, "c": 3}'); | ["a", "b", "c"]  |
| [JSON_EXISTS](../../sql-functions/json-functions/json-processing-functions/json_exists.md)   | 查询 JSON 对象中指定路径（json_path）是否存在满足特定条件的值。      | SELECT JSON_EXISTS(PARSE_JSON('{"a": {"b": 1}}'), '$.a.b'); | 1  |
